...
const App = () => {
  ...
  const handleSelectPolicy = (selectedPolicy) => {
    ...
    setCurrentProcessIndex(0); // Reset the current process index
  };
  ...
  const runProcess = () => {
    setProcesses((prevProcesses) => {
      ...
      if (policy === 'RR') {
        const runningProcessIndex = updatedProcesses.findIndex(p => p.status === 'Running');
        const nextIndex = runningProcessIndex !== -1 ? (runningProcessIndex + 1) % updatedProcesses.length : 0;
        let remainingQuantum = quantum;
  
        if (runningProcessIndex !== -1) {
          // Continue running the current process
          const runningProcess = updatedProcesses[runningProcessIndex];
          runningProcess.burstTime -= 1;
          remainingQuantum = runningProcess.quantumLeft - 1;
  
          if (runningProcess.burstTime <= 0) {
            runningProcess.status = 'Terminated';
            deallocateMemory(runningProcess);
            remainingQuantum = quantum; // Reset quantum for next process
          } else if (remainingQuantum <= 0) {
            runningProcess.status = 'Ready';
            remainingQuantum = quantum; // Reset quantum for next process
          } else {
            runningProcess.quantumLeft = remainingQuantum;
          }
        }
  
        const nextProcess = updatedProcesses.find((p, index) => index >= nextIndex && p.status === 'Ready');
        if (nextProcess) {
          nextProcess.status = 'Running';
          nextProcess.quantumLeft = remainingQuantum; // Assign remaining quantum or reset
          setCurrentProcessIndex(updatedProcesses.indexOf(nextProcess));
        }
